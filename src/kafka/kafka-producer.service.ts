...
export class KafkaProducerService implements OnModuleInit, OnModuleDestroy {
  ...
  private readonly topic: string = process.env.KAFKA_TOPIC || 'token-price-updates';
  ...
  async onModuleInit(): Promise<void> {
    ...
    if (!this.enabled) {
      this.logger.warn('Kafka disabled (KAFKA_ENABLED!=true and NODE_ENV!=production). Skipping connect.');
      return;
    }
    ...
    try {
      ...
      this.logger.log(`Connected to Kafka [${brokers.join(', ')}], topic=${this.topic}`);
    } catch (err) {
      this.enabled = false;
      this.logger.error(`Kafka init failed: ${(err as Error).message}. Continuing without Kafka.`);
    }
  }
  ...
  async sendPriceUpdateMessage(message: TokenPriceUpdateMessage): Promise<void> {
    try {
      ...
      // Простая валидация
      if (!message.tokenId || !message.symbol || message.oldPrice < 0 || message.newPrice < 0) {
