...
describe("TokenPrice", () => {
  describe("create", () => {
    ...
    it("should reject negative prices", () => {
      expect(() => TokenPrice.create(-100)).toThrow(
        "Token price must be positive"
      );
    });

    it("should reject zero price", () => {
      expect(() => TokenPrice.create(0)).toThrow(
        "Token price must be positive"
      );
    });
    ...
    it("should reject prices exceeding safe integer limit", () => {
      ...
      expect(() => TokenPrice.create(tooLarge)).toThrow(
        "Token price exceeds safe integer limit"
      );
    });
    ...
    it("should reject prices with more than 8 decimal places", () => {
      expect(() => TokenPrice.create(1000.123456789)).toThrow(
  ...
  describe("arithmetic operations", () => {
    ...
    it("should validate arithmetic results", () => {
      ...
      expect(() => price2.subtract(price1)).toThrow(
        "Token price must be positive"
      );
    });
  });
  ...
  describe("toString", () => {
    it("should convert to string correctly", () => {
      const price = TokenPrice.create(1000.5);
