...
export class PricingInterfaceModule implements OnModuleInit {
  ...
  async onModuleInit() {
    try {
      ...
      const autoSeedOnStartup =
        (process.env.AUTO_SEED_ON_STARTUP || "true") === "true";

      // Автоматическое сидирование только в development и только если включено
      if (!isTestEnv && autoSeedOnStartup) {
        this.logger.log(
          "Auto-seeding enabled, starting initial data seeding..."
        );
        await this.seedHandler.execute(new SeedInitialDataCommand("system"));
      } else {
        this.logger.log(
          "Auto-seeding disabled or test environment, skipping seed"
        );
      }
    } catch (error) {
