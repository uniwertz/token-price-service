...
export class UpdateTokenPricesHandler {
   ...
   * 4. Сохранить изменения в БД батчем (1 транзакция на 100 токенов)
   * 5. Опубликовать события в Kafka (ПОСЛЕ успешного сохранения в БД)
  ...
  async execute(_cmd: UpdateTokenPricesCommand): Promise<void> {
    ...
    try {
      ...
      await this.repo.processAll(async (batch) => {
        ...
        // Сначала сохраняем изменения в БД батчем (1 транзакция вместо 100 запросов)
        if (tokensToSave.length > 0) {
          await this.repo.saveBatch(tokensToSave);
        }

        // Затем публикуем ВСЕ события батча одним вызовом (1 запрос в Kafka вместо 100)
        // Публикация осуществляется только после успешного коммита в БД,
        // чтобы избежать рассинхронизации между событием и состоянием хранилища
