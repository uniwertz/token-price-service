...
import { EXTERNAL_PRICE_SERVICE_PORT, ExternalPriceServicePort } from '@contexts/pricing/domain/repositories/external-price-service.port';
import { DOMAIN_EVENT_BUS, DomainEventBus } from '@shared/kernel/domain-event-bus.port';
...
export class UpdateTokenPricesHandler {
	...
	constructor(
		...
		@Inject(EXTERNAL_PRICE_SERVICE_PORT) private readonly externalPriceService: ExternalPriceServicePort,
	...
	async execute(_cmd: UpdateTokenPricesCommand): Promise<void> {
		...
		try {
			...
			for (const token of tokens) {
				try {
					const newPrice = await this.externalPriceService.getPriceForToken({ id: token.id, symbol: token.symbol });
					...
					errorCount++;
					this.logger.error(`Failed to update price for token ${token.id}`, (error as Error).stack, {
						tokenId: token.id,
						symbol: token.symbol
					});
				}
			...
			this.logger.error('Failed to update token prices', (error as Error).stack);
			this.telemetry.recordMetric({
