...
export class PrismaInitialDataRepository implements InitialDataRepositoryPort {
  ...
  async seed(author: string): Promise<void> {
    ...
    const ethToken = await this.prisma.token.create({
      data: {
        contractAddress: Buffer.from([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09]),
        symbol: 'ETH',
    ...
    const btcToken = await this.prisma.token.create({
      data: {
        contractAddress: Buffer.from([0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19]),
        symbol: 'BTC',
        displayName: 'Bitcoin',
        decimalPlaces: 8,
        isNativeToken: true,
    ...
    const solToken = await this.prisma.token.create({
      data: {
        contractAddress: Buffer.from([0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29]),
        symbol: 'SOL',
        displayName: 'Solana',
        decimalPlaces: 9,
        isNativeToken: true,
