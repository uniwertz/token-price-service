...
- **Что сделано**:
  ...
  - Для `price (decimal)` внедрено безопасное представление: данные хранятся/обрабатываются как `string` либо через `ValueTransformer`/big number библиотеку.
...
- **Что сделано**:
  - Внедрён `@nestjs/schedule` (`Cron`/`Interval`) с взаимным исключением выполнения (mutex/«шторка») либо эквивалентной защитой от конкурентных запусков.
  - Интервал сделан конфигурируемым; при необходимости обработка выполнена батчами и с ограничением параллелизма.
  - Запуск сервиса перенесён из `AppModule.onModuleInit` в отдельный orchestrator/явный шаг и не смешивается с сидингом.
